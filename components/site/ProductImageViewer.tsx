...
/**
 * Large image + price + navigation between a product's other items, per
 * the catalog spec — plus the product-level details (description, specs,
 * availability, delivery, Call/WhatsApp) the catalog already required
 * before this. Those didn't stop being wanted just because the newer spec
 * that added multi-item navigation didn't re-list them.
 */
export function ProductImageViewer({
  ...
  onNavigate: (item: ProductItem) => void;
  onClose: () => void;
}) {
  const panelRef = useRef<HTMLDivElement>(null);
  const activeItems = useMemo(() => product.items.filter((item) => item.status === 'ACTIVE'), [product.items]);
  const index = activeItems.findIndex((item) => item.id === activeItem.id);

  const goTo = (nextIndex: number) => {
    const wrapped = (nextIndex + activeItems.length) % activeItems.length;
    onNavigate(activeItems[wrapped]);
  };
  ...
  const waMessage = buildWhatsAppUrl(
    `Hi! I'm interested in "${product.name}" (${formatPrice(activeItem.price)}). ${product.description} — could you share more details?`
  );
      ...
      >
        ...
        <div className="relative">
          {activeItem.image ? (
            ...
            <ProductSwatch
              ...
              className="aspect-auto h-64 rounded-t-sm sm:h-full sm:rounded-l-sm sm:rounded-tr-none"
            />
          )}
        ...
        <div className="flex flex-col gap-4 p-6 sm:p-8">
          <div>
            ...
            <p className="mt-2 font-serif text-2xl text-maroon">{formatPrice(activeItem.price)}</p>
          </div>

          <p className="font-sans text-sm leading-relaxed text-earth">{product.description}</p>

          {product.specifications.length > 0 && (
            <dl className="grid grid-cols-1 gap-x-4 gap-y-1.5 border-y border-gold/30 py-4 font-sans text-sm">
